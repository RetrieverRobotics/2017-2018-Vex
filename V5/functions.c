...
float scalePotRToL(float rArmPot) {
	return .8937 * SensorValue[armPotR] - 13;
...
void driveSlow(float distance, bool incremental = true){
...
void updatePIDVar(PIDStruct *PIDVar) {
	...
	if(PIDVar->enabled) {
		...
		proportional = PIDVar->kP * PIDVar->error; // P
		// Check active zone for I
		if (fabs(PIDVar->error) <= PIDVar->integralActiveZone) {
			PIDVar->integral += PIDVar->kI * PIDVar->error * PIDVar->loopTime; // I
...
task drivePIDTask() {
	...
	while(true) {
		...
		if(driveMode == POINT_TURN) {
			...
			// writeDebugStreamLine("%f\t%f\t%f", drivePID.input, getRDriveEnc(), SensorValue[driveREnc]);
			// writeDebugStreamLine("%i\t%i",drivePID.output,gyroPID.output);
			tankDrive(
			...
			tankDrive(
				lim127(lim127(drivePID.output, 60) - gyroPID.output, 60),
				lim127(lim127(drivePID.output, 60) + gyroPID.output, 60)
			);
		}
...
task armPIDTask() {
	...
	while(true) {
		...
		armCrossCouplePID.output = sgn(armCrossCouplePID.output) * pow(abs(armCrossCouplePID.output), 1.5) / 10;

		if(armCrossCouplePID.debug){
			writeDebugStreamLine("%i",armCrossCouplePID.output);
		}
		...
		if (armMode == ARM_PID_CONTROL) {
			...
			// arm cross couple can compensate no more than 127
			// if(armCrossCouplePID.input > 50){
			// 	armCrossCouplePID.output = -60;
			// }
			// else if(armCrossCouplePID.input < -50){
			// 	armCrossCouplePID.output = 60;
			// }
			// else{
			// 	armCrossCouplePID.output = 0;
