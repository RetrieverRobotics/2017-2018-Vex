...
#pragma systemFile // get rid of unused function errors
...
task drivePIDTask(){
    ...
    // average of left and right for average distance travelled.
		...
		motor[driveL1] = lim127(drivePID.output) - lim127(gyroPID.output);
		motor[driveL2] = lim127(drivePID.output) - lim127(gyroPID.output);
		motor[driveR1] = lim127(drivePID.output) + lim127(gyroPID.output);
		motor[driveR2] = lim127(drivePID.output) + lim127(gyroPID.output);
